...
const userRegister = async function (){
  try {
    ((phoneNum.value && code.value && password.value && isChecked.value) &&
    await store.dispatch('getRegisterUser',{phone:phoneNum.value,code:code.value,password:password.value}))
    router.push('/login')
  }catch (error){
    alert(error.message)
  }
  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        ...
        <button style="height: 38px;width: 70px;margin-left: 10px" @click="getCode(phoneNum)">获取验证码</button>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
