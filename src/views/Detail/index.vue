...
const changeActive = function (spuSaleAttrValue,spuSaleAttrList){
  //需要拿到装该对象的数组
...
const changeNum = function (){
  if (isNaN(skuNum.value) || skuNum.value <= 0){
    skuNum.value = 1
  }else {
...
const addShopCart = async function (){
  //派发请求 调用仓库中的 addOrUpdateShopCart() 函数，而addOrUpdateShopCart()函数的返回值为一个 Promise
//  let result = await store.dispatch('addOrUpdateShopCart',{skuId: route.params.skuid,skuNum:skuNum.value})
//  console.log(result) 如果成功就返回 ok
  try {
    ...
    sessionStorage.setItem('skuInfo',JSON.stringify(store.getters.skuInfo))
    sessionStorage.setItem('spuSaleAttrList',JSON.stringify(store.getters.spuSaleAttrList))
  }catch (error){
...
onMounted(()=>{
  store.dispatch('detailInfoList',route.params.skuid)
})
</script>
    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果 (放大镜中的图片和小图列表里的是相同的数据)-->
          <Zoom :sku-image-list="skuImageList"/>
          <!-- 小图列表 -->
          <ImageList :sku-image-list="skuImageList"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
...
<!--                <dd changepirce="0" class="active">金色</dd>-->
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" ref="input" @change="changeNum">
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" :class="{disabled:skuNum == 1}" @click="skuNum > 1?skuNum --:skuNum = 1">-</a>
              </div>
              <div class="add">
